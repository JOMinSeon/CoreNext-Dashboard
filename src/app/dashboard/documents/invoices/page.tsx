...
import { useDocumentStore, Invoice, DocumentStatus } from "@/store/document-store"
...
import { Plus, MoreHorizontal, Pencil, Trash2, Search, Receipt, Download } from "lucide-react"
import { useForm, useFieldArray } from "react-hook-form"
import { zodResolver } from "@hookform/resolvers/zod"
import { z } from "zod"
import { DocumentItemSchema, createInvoiceSchema } from "@/lib/schemas"
...
export default function InvoicesPage() {
  const { invoices, addInvoice, updateInvoice, deleteInvoice } = useDocumentStore()
  ...
  const onSubmit = (data: InvoiceForm) => {
    ...
    if (editingInvoice) {
      updateInvoice(editingInvoice.id, processedData)
    } else {
      addInvoice(processedData)
    }
  ...
  const handleDownload = (invoice: Invoice) => {
    ...
    exportInvoiceToPDF(invoice, client, invoice.items)
  }
    ...
    <div className="p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="mt-2 text-gray-500">
            총 {invoices.length}개의 거래명세서가 있습니다
          </p>
        </div>
          ...
          {filteredInvoices.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">
                {searchQuery ? "검색 결과가 없습니다" : "아직 등록된 거래명세서가 없습니다"}
              </p>
              {!searchQuery && (
                <Button onClick={openAddDialog}>
                  <Plus className="h-4 w-4 mr-2" />
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem className="text-red-600" onClick={() => handleDelete(invoice)}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editingInvoice ? "거래명세서 수정" : "거래명세서 작성"}
            </DialogTitle>
            <DialogDescription>
              {editingInvoice ? "거래명세서 정보를 수정합니다" : "새 거래명세서를 작성합니다"}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid gap-2">
                  <Label htmlFor="dueDate">결제기한</Label>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>거래명세서 삭제</DialogTitle>
            <DialogDescription>
              이 거래명세서를 삭제하시겠습니까?
            </DialogDescription>
          </DialogHeader>
